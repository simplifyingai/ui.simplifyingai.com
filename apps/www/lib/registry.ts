...
function buildPathMappings(
  ...
  files.forEach((file) => {
    if (file.path && file.target) {
      // Extract the source path relative to registry/simplifying-ai/
      const match = file.path.match(/registry\/simplifying-ai\/(.+)$/)
      if (match) {
...
export function fixImport(content: string, pathMappings: Map<string, string>) {
  ...
  content = content.replaceAll(
    "@/registry/simplifying-ai/ui/",
    "@/components/ui/"
  )

  // Fix example imports
  content = content.replaceAll(
    "@/registry/simplifying-ai/examples/",
    "@/components/examples/"
  )

  // Fix hook imports
  content = content.replaceAll("@/registry/simplifying-ai/hooks/", "@/hooks/")

  // Fix lib imports
  content = content.replaceAll("@/registry/simplifying-ai/lib/", "@/lib/")
