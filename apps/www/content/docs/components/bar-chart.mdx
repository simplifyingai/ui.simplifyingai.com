---
title: Bar Chart
description: A flexible bar chart component for visualizing categorical data with support for multiple variants including horizontal, stacked, grouped, and interactive layouts.
featured: true
component: true
---

<ComponentPreview
  name="bar-chart-demo"
  description="A flexible bar chart with support for vertical, horizontal, stacked, and grouped layouts."
/>

## About

The Bar Chart component is built on top of [Recharts](https://recharts.org/) and provides a simple, declarative way to create bar charts. It supports various configurations including:

- **Vertical and horizontal** orientations
- **Single and multiple** data series
- **Stacked and grouped** bar layouts
- **Custom tooltips** with theme-aware styling
- **Responsive design** that adapts to container width

## Emphasis and annotation

`activeLabel` (or `activeIndex`) keeps one bar vivid and drops the rest to
`mutedColor`; `pattern` / `mutedPattern` give either group a texture. A
`referenceLine` takes a literal value or a statistic (`"avg"`, `"min"`,
`"max"`).

Nothing about the annotation look is fixed. `labelVariant` picks between
`plain` (text only — the default, since a filled chip floating over a plot
reads as a stuck tooltip), `pill` and `flag`; `activeTickVariant` picks
`plain`, `pill` or `underline`. Set them per chart, set them once on a
`ChartThemeProvider`, or pass your own renderer to `label` and skip the
built-ins entirely.

```tsx
<BarChart
  data={data}
  activeLabel="May"
  referenceLine={{ value: "avg", label: "Average", labelVariant: "flag" }}
  activeTickVariant="pill"
/>
```

<ComponentPreview
  name="bar-chart-emphasis-demo"
  description="One emphasised bar against hatched neighbours, with an average reference line."
/>

## Mobile, zoom and overflow

Every category chart measures **its own box**, not the viewport, so a chart
in a narrow card behaves like a chart on a phone. `aspectRatio` and
`legendPosition` take a value per container-width breakpoint, `minHeight`
stops a phone-width plot collapsing to a strip, and the category ticks thin
themselves out until the labels stop colliding.

When the categories outnumber the pixels there are only two honest options,
and `overflow` picks between them: `compress` squeezes them in, `scroll`
gives each one `minCategorySize` and lets the plot overflow — sideways for
vertical bars, downwards for horizontal ones.

Zoom works from every input: drag to box-select, ⌘/ctrl + wheel (or
`wheelZoom` for a bare wheel), two-finger pinch, one-finger pan once zoomed,
double-tap to reset, plus the +/−/reset buttons. It turns itself off while
the plot is scrolling, since scroll already owns that axis.

<ComponentPreview
  name="bar-chart-scroll-demo"
  description="Forty-eight categories: scrolling sideways above, zoomable below."
/>

| Prop | Type | Default | Description |
| --- | --- | --- | --- |
| `aspectRatio` | `number \| { base?, sm?, md?, lg?, xl? }` | `2` | Width ÷ height, per container width. |
| `minHeight` | `number` | `180` | Floor on plot height, px. |
| `overflow` | `"compress" \| "scroll"` | `"compress"` | What to do when categories outnumber pixels. |
| `minCategorySize` | `number` | `32` | Room one category needs before `scroll` overflows. |
| `zoomable` | `boolean` | `true` | Drag / pinch / wheel zoom. |
| `wheelZoom` | `boolean` | `false` | Let a bare wheel zoom, not just ⌘/ctrl + wheel. |
| `showZoomControls` | `boolean` | `true` | The +/−/reset buttons. |
| `xAxisInterval` | `number \| "preserveStartEnd"` | auto | Ticks skipped between labels. |

## Installation

<CodeTabs>

<TabsList>
  <TabsTrigger value="cli">CLI</TabsTrigger>
  <TabsTrigger value="manual">Manual</TabsTrigger>
</TabsList>
<TabsContent value="cli">

```bash
npx shadcn@latest add https://ui.simplifyingai.com/r/bar-chart.json
```

</TabsContent>

<TabsContent value="manual">

<Steps>

<Step>Copy and paste the following code into your project.</Step>

<ComponentSource name="bar-chart" title="components/ui/charts/bar-chart.tsx" />

<Step>Update the import paths to match your project setup.</Step>

</Steps>

</TabsContent>

</CodeTabs>

## Usage

```tsx showLineNumbers
import { BarChart } from "@/components/ui/charts"

const data = [
  { label: "Jan", value: 4000 },
  { label: "Feb", value: 3000 },
  { label: "Mar", value: 5000 },
]

export function MyChart() {
  return <BarChart data={data} />
}
```

## Examples

### Default

The default bar chart displays categorical data with sharp-edged bars, horizontal grid lines, and an interactive tooltip on hover.

<ComponentPreview name="bar-chart-demo" />

### With Labels

Use the `showLabel` prop to display values directly on top of each bar. This is useful when you want to show exact values without requiring users to hover.

<ComponentPreview name="bar-chart-label-demo" />

```tsx showLineNumbers
<BarChart data={data} showLabel labelPosition="top" />
```

### Horizontal

Horizontal bar charts are ideal for displaying data with long category names or when you have many categories. Use `layout="vertical"` to switch orientation (Recharts naming convention).

<ComponentPreview name="bar-chart-horizontal-demo" />

```tsx showLineNumbers
<BarChart data={data} layout="vertical" />
```

### Multiple / Grouped

Display multiple data series side-by-side to compare different metrics across categories. Use the `MultiBarChart` component with a `series` configuration.

<ComponentPreview name="bar-chart-multiple-demo" />

```tsx showLineNumbers
import { MultiBarChart } from "@/components/ui/charts"

const data = [
  { label: "Jan", desktop: 186, mobile: 80 },
  { label: "Feb", desktop: 305, mobile: 200 },
]

const series = [
  { name: "Desktop", dataKey: "desktop", color: "#2563eb" },
  { name: "Mobile", dataKey: "mobile", color: "#60a5fa" },
]

<MultiBarChart data={data} series={series} />
```

### Stacked

Stacked bar charts show the cumulative total of multiple series. Add a `stackId` to each series to group them into stacks.

<ComponentPreview name="bar-chart-stacked-demo" />

```tsx showLineNumbers
const series = [
  { name: "Desktop", dataKey: "desktop", color: "#2563eb", stackId: "stack" },
  { name: "Mobile", dataKey: "mobile", color: "#60a5fa", stackId: "stack" },
]

<MultiBarChart data={data} series={series} />
```

### Negative Values

Bar charts can display both positive and negative values. Bars automatically extend above or below the zero reference line.

<ComponentPreview name="bar-chart-negative-demo" />

### Interactive

Create interactive charts with state-controlled data series. Users can toggle between different views of the data.

<ComponentPreview name="bar-chart-interactive-demo" />

### Mixed (Bar + Line)

Combine bars with a line chart to show both absolute values and trends. Uses Recharts' `ComposedChart` component.

<ComponentPreview name="bar-chart-mixed-demo" />

### Active Bar

Highlight a specific bar using a custom shape with a dashed border effect. Useful for emphasizing a selected or current data point.

<ComponentPreview name="bar-chart-active-demo" />

```tsx showLineNumbers
import { Bar, BarChart, Rectangle } from "recharts"

const ACTIVE_INDEX = 2

<Bar
  dataKey="sales"
  radius={12}
  shape={({ index, fill, ...props }) =>
    index === ACTIVE_INDEX ? (
      <Rectangle
        {...props}
        fill={fill}
        fillOpacity={0.6}
        stroke={fill}
        strokeWidth={2}
        strokeDasharray="4 4"
      />
    ) : (
      <Rectangle {...props} fill={fill} />
    )
  }
/>
```

## API Reference

### BarChart

| Prop             | Type                                        | Default        | Description                                    |
| ---------------- | ------------------------------------------- | -------------- | ---------------------------------------------- |
| `data`           | `BarChartDataPoint[]`                       | required       | Array of data points to display                |
| `color`          | `string`                                    | `"#2563eb"`    | Fill color for bars                            |
| `className`      | `string`                                    | -              | Additional CSS classes                         |
| `showGrid`       | `boolean`                                   | `true`         | Show horizontal grid lines                     |
| `showTooltip`    | `boolean`                                   | `true`         | Enable tooltip on hover                        |
| `showLabel`      | `boolean`                                   | `false`        | Show value labels on bars                      |
| `labelPosition`  | `"top" \| "center" \| "bottom" \| "inside"` | `"top"`        | Position of value labels                       |
| `valueFormatter` | `(value: number) => string`                 | -              | Format displayed values                        |
| `labelFormatter` | `(label: string) => string`                 | -              | Format category labels                         |
| `layout`         | `"vertical" \| "horizontal"`                | `"horizontal"` | Bar orientation (`vertical` = horizontal bars) |
| `barRadius`      | `number`                                    | `4`            | Corner radius of bars                          |
| `aspectRatio`    | `number`                                    | `2`            | Width to height ratio                          |
| `yAxisWidth`     | `number`                                    | `48`           | Width reserved for Y-axis labels               |

### MultiBarChart

| Prop             | Type                         | Default        | Description                        |
| ---------------- | ---------------------------- | -------------- | ---------------------------------- |
| `data`           | `MultiBarChartDataPoint[]`   | required       | Array of data points               |
| `series`         | `MultiBarChartSeries[]`      | required       | Configuration for each data series |
| `className`      | `string`                     | -              | Additional CSS classes             |
| `showGrid`       | `boolean`                    | `true`         | Show horizontal grid lines         |
| `showTooltip`    | `boolean`                    | `true`         | Enable tooltip on hover            |
| `showLegend`     | `boolean`                    | `true`         | Show legend below chart            |
| `valueFormatter` | `(value: number) => string`  | -              | Format displayed values            |
| `labelFormatter` | `(label: string) => string`  | -              | Format category labels             |
| `layout`         | `"vertical" \| "horizontal"` | `"horizontal"` | Bar orientation                    |
| `barRadius`      | `number`                     | `4`            | Corner radius of bars              |
| `aspectRatio`    | `number`                     | `2`            | Width to height ratio              |
| `yAxisWidth`     | `number`                     | `48`           | Width reserved for Y-axis labels   |

## Data Types

```tsx
// Single series data point
interface BarChartDataPoint {
  label: string // Category name (e.g., "January")
  value: number // Numeric value
  fill?: string // Optional: override color for this bar
}

// Multiple series data point
interface MultiBarChartDataPoint {
  label: string // Category name
  [key: string]: string | number // Dynamic keys for each series
}

// Series configuration
interface MultiBarChartSeries {
  name: string // Display name in tooltip/legend
  dataKey: string // Key in data object
  color: string // Bar fill color
  stackId?: string // Optional: group into stack
}
```

## Customization

### Custom Colors

Pass a `color` prop or use per-bar colors via the `fill` property in data:

```tsx showLineNumbers
const data = [
  { label: "Success", value: 80, fill: "#22c55e" },
  { label: "Warning", value: 15, fill: "#eab308" },
  { label: "Error", value: 5, fill: "#ef4444" },
]

<BarChart data={data} />
```

### Custom Tooltip

For advanced tooltip customization, use the Recharts components directly:

```tsx showLineNumbers
import { Bar, BarChart, Tooltip } from "recharts"

;<BarChart data={data}>
  <Tooltip
    content={({ active, payload }) => (
      <div className="custom-tooltip">{/* Your custom content */}</div>
    )}
  />
  <Bar dataKey="value" />
</BarChart>
```

## Accessibility

- Charts include proper ARIA labels
- Tooltips are keyboard accessible
- Color schemes maintain sufficient contrast
- Grid lines help users track values across the chart

## Notes

- Bar charts are best suited for comparing categorical data across different groups
- Use horizontal layout (`layout="vertical"`) when dealing with long category labels
- Stacked bar charts are ideal for showing part-to-whole relationships over categories
- The `MultiBarChart` component automatically handles legend generation and color coordination
- For time-series data with many points, consider using a line or area chart instead
- Bar radius can be adjusted for different visual styles (sharp edges vs rounded corners)
