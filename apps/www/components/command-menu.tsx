...
import {
  ...
  CommandItem,
  CommandList,
} from "@/registry/simplifying-ai/ui/command"
import {
  Dialog,
  DialogContent,
  DialogDescription,
  ...
  DialogTitle,
  DialogTrigger,
} from "@/registry/simplifying-ai/ui/dialog"
...
export function CommandMenu({
  ...
  const handlePageHighlight = React.useCallback(
    (isComponent: boolean, item: { url: string; name?: React.ReactNode }) => {
      if (isComponent) {
        ...
        setCopyPayload(
          `${packageManager} dlx @simplifying-ai/cli@latest add ${componentName}`
        )
      } else {
  ...
  const handleBlockHighlight = React.useCallback(
    (block: { name: string; description: string; categories: string[] }) => {
      ...
      setCopyPayload(
        `${packageManager} dlx @simplifying-ai/cli@latest add ${block.name}`
      )
    },
